...
Vagrant::Config.run do |config|
    ...
    config.vm.define :db do |db|
        db.vm.box = "debian-squeeze"
        # Debian Squeeze amd64 (Chef 10.24.4, Puppet 3.1.1, VirtualBox 4.2.12)
        # https://dl.dropboxusercontent.com/u/13054557/vagrant_boxes/debian-squeeze.box
        db.vm.box_url = "~/debian-squeeze.box"

        # Boot with a GUI so you can see the screen. (Default is headless)
        # config.vm.boot_mode = :gui
        db.vm.network :hostonly, "192.168.56.12"
        db.vm.network :bridged, :bridge => "eth0"
        db.vm.forward_port 80, 8082

        db.vm.share_folder "www", "/var/www", "/mnt/db-www", :create => true
